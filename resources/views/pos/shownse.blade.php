...
@section('content')
    ...
    @endif
    @php
        $totalModal = $sale->items->sum(fn($item) => $item->batches->sum(fn($b) => $b->cost_price * $b->qty));
        $totalHargaJual = $sale->items->sum('subtotal');
    @endphp
                ...
                <tbody>
                    @foreach ($sale->items as $item)
                        <tr @class([
                            ...
                            <td class="text-end">
                                {{ number_format($item->batches->sum(fn($b) => $b->cost_price * $b->qty)) }}</td>
                ...
                <div class="col-md-4 col-sm-6">
                    <table class="table table-sm">
                        <tr>
                            ...
                            <td class="text-end">{{ number_format($totalHargaJual) }}</td>
                        </tr>
                        <tr class="fw-bold">
                            <th>Total Harga Modal</th>
                            <td class="text-end">{{ number_format($totalModal) }}</td>
                        </tr>
                        <tr class="fw-bold">
                            <th>Laba / Rugi</th>
                            <td class="text-end {{ $totalHargaJual - $totalModal >= 0 ? 'text-success' : 'text-danger' }}">
                                {{ number_format($totalHargaJual - $totalModal) }}
                            </td>
                        </tr>
                    </table>
                </div>
                ...
                @if ($sale->status == 'paid' && $sale->refunds->count() === 0)
                    <form method="POST" class="form-inline mb-0" action="{{ route('sales.void', $sale) }}"
                        ...
                        <button class="btn btn-outline-danger">
                            <i class="bi bi-trash"></i> Void Distribusi NSE
                        </button>
                    </form>
                @endif
            </div>
                ...
                <form method="POST" action="{{ route('sales.exchange', $sale) }}">
                    ...
                    <div class="modal-body">
                        ...
                        <input type="number" name="qty" class="form-control" value="1" min="1" required
                            onkeyup="updatePrice()" autocomplete="off">
