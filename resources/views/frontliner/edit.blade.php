                            ...
                            <div class="card-body">
                                ...
                                <div class="row mb-3">
                                        ...
                                        <select name="akundebet" class="form-select">
                                            @foreach ($kas as $dt)
                                                <option value="{{ $dt->kode }}"
                                                    {{ isset($data) && $data->akun_debet == $dt->kode ? 'selected' : '' }}>
                                                    {{ $dt->kode }} - {{ $dt->nama }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-md-4 form-group">
                                        ...
                                        <textarea name="ket" class="form-control" rows="2">{{ $data->catatan ?? '' }}</textarea>
                                    </div>
                                </div>
...
@push('scripts')
    <script>
        function simpanTransaksi() {
            ...
            fetch("{{ route('frontliner.update', Crypt::encryptString($data->Id)) }}", {
                    method: "POST",
                    body: formData,
                    headers: {
                ...
                .then(res => {
                    if (res.status) {
                        Swal.fire("Sukses", res.msg, res.status ? "success" : "error").then(() => {
                            if (res.status) {
