            ...
            <div class="card rounded-4 p-2">
                <div class="card-header d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                    <div class="d-flex align-items-start">
                        ...
                        <div>
                            <h5 class="fw-bold mb-0" style="color:#7c3aed">Laporan Member & Total Poin</h5>
                ...
                <div id="dataContainer" class="p-3">
                    <div class="text-center text-muted py-5">
                        <i class="material-icons-outlined" style="font-size:48px;opacity:.3">stars</i>
                        <p class="mt-2">Klik Tampilkan untuk memuat data laporan member</p>
                    </div>
                </div>
            </div>
        </div>
...
@push('scripts')
    <script>
        ...
        $('#btnTampilkan').on('click', function() {
            $('#dataContainer').html(
                '<div class="text-center py-5"><div class="spinner-border text-primary"></div><p class="mt-2 text-muted">Memuat data member...</p></div>'
            );

            $.ajax({
                url: '{{ route('laporan.member.data') }}',
                type: 'POST',
                data: getParams(),
        ...
        $('#btnExport').on('click', function() {
            ...
            window.location.href = '{{ route('laporan.member.export') }}?' + qs.toString();
        });
