            ...
            <div class="card rounded-4 p-2">
                ...
                <div class="card-body">
                    <ul class="nav nav-tabs" id="myTab" role="tablist">
                        <li class="nav-item"><a class="nav-link active" id="users-tab" data-bs-toggle="tab" href="#users"
                                role="tab" aria-controls="users" aria-selected="true">User</a></li>
                        <li class="nav-item"><a class="nav-link" id="menus-tabs" data-bs-toggle="tab" href="#menus"
                                role="tab" aria-controls="menus" aria-selected="false">Menu</a></li>
                    </ul>
                                ...
                                <div class="col-6">
                                    <form id="addUserForm">
                                        ...
                                        <div class="text-end">
                                            ...
                                            <button type="button" class="btn btn-primary btn-sm" id="saveUserBtn"><i
                                ...
                                <table class="table table-striped datatables" id="pegawai-table">
                                    ...
                                    <tbody>
                                        @forelse($role->users as $index => $row)
                                            <tr>
                                                <td>
                                                    <img class="b-r-10" width="35" height="35"
                                                        src="{{ asset('assets/images/avatars/11.png') }}" alt="">
                                                </td>
                                                <td class="align-middle">{{ optional($row->pengguna)->email }}</td>
                                                <td class="align-middle">{{ optional($row->pengguna)->name }}</td>
                                                <td class="text-center align-middle">
                                                    <a href="javascript:void(0);" class="delete-btn"
                                                        data-id="{{ Crypt::encryptString($row->id) }}"
                                                        title="Melihat detail dari role"><i
                                        ...
                                        <tr>
                                            <td class="text-center">
                                                <div class="parent-icon"><i
                                                        class="material-icons-outlined">{{ $menu->icon }}</i></div>
                                            </td>
                                            <td>{{ $menu->nama }}</td>
                                            <td class="text-center" width="10%">
                                                {{ $menu->stts == 'Y' ? 'Aktif' : 'Non Aktif' }}</td>
                                            <td class="text-center" width="10%">
                                                <div class="form-check checkbox mb-0 d-flex justify-content-center">
                                                    <input class="form-check-input" id="checkbox{{ $menu->id }}"
                                                        name="has_access" type="checkbox" value="{{ $menu->id }}"
                                                        {{ $menu->has_access == 1 ? 'checked=""' : '' }}>
                                                    <label class="form-check-label"
                                                        for="checkbox{{ $menu->id }}"></label>
                                                </div>
                                            </td>
                                        </tr>
                                        @if ($menu->children->isNotEmpty())
                                            @foreach ($menu->children as $child)
                                                <tr>
                                                    ...
                                                    <td class="text-center" width="10%">
                                                        {{ $child->stts == 'Y' ? 'Aktif' : 'Non Aktif' }}</td>
                                                    <td class="text-center" width="10%">
                                                        <div
                                                            class="form-check checkbox mb-0 d-flex justify-content-center">
                                                            <input class="form-check-input"
                                                                id="checkbox{{ $child->id }}" name="has_access"
                                                                type="checkbox" value="{{ $child->id }}"
                                                                {{ $child->has_access == 1 ? 'checked=""' : '' }}>
                                                            <label class="form-check-label"
                                                                for="checkbox{{ $child->id }}"></label>
                                                        </div>
                                                    </td>
                                                </tr>
                                            @endforeach
