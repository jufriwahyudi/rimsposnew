...
return new class extends Migration
{
    ...
    public function up(): void
    {
        if (Schema::hasTable('menu_list')) {
            ...
            $exists = DB::table('menu_list')->where('routename', 'cash-registers.index')->exists();

            if (!$exists) {
                DB::table('menu_list')->insert([
                    'nama'       => 'Laporan Shift Kasir',
                    'routename'  => 'cash-registers.index',
                    'icon'       => '',
                    'id_parent'  => $parentId,
                    'jnsmenu'    => 'child',
                    'urutan'     => $maxUrutan + 1,
    ...
    public function down(): void
    {
        if (Schema::hasTable('menu_list')) {
            DB::table('menu_list')
                ->where('routename', 'cash-registers.index')
                ->delete();
        }
    }
};
